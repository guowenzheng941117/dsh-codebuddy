 ...
 * 窗口、最大输出、视觉能力、reasoning 支持档位（reasoning.supportedEfforts）
 * 全部按上游真实数据热替换（默认每 6 小时，内存换表不重启）；失败沿用
 * models.json 静态表，连续失败按 30s→5min 指数退避。
...
/** 实测接受 reasoning_effort 的模型（仅当上游未声明 reasoning.supportedEfforts
 *  时作为保守回退；models.json 里同名条目的声明最优先）。
 *  hy4-preview/-x 上游声明仅支持 high，由 toModelEntry 走上游档位，不在此列。 */
const REASONING_KNOWN = new Set(['hy3', 'hy3-x', 'deepseek-v4-pro', 'deepseek-v4-flash', 'kimi-k2-thinking'])
...
/** 上游模型条目 → 插件模型条目；models.json 同名条目的 reasoningEfforts / description 声明优先 */
function toModelEntry(m) {
  ...
  // 上游 reasoning.supportedEfforts = 官方声明的支持档位（词汇 low/high/max）；
  // 未声明才回退保守白名单，避免发出上游不收的档位（如 hy4-preview 仅支持 high）
  ...
  if (stat) reasoningEfforts = stat.reasoningEfforts
  else if (upstreamEfforts.length) reasoningEfforts = upstreamEfforts
  else reasoningEfforts = REASONING_KNOWN.has(m.id) ? ['low', 'high'] : null
  return {
    id: m.id,
    name: m.name || m.id,
    ...
    reasoningEfforts,
    input: m.supportsImages && !m.disabledMultimodal ? ['text', 'image'] : ['text'],
  }
}
