...
const BUILTIN_CAPABILITIES = {
  'hy3':               { efforts: ['low', 'high', 'max'], vision: false, context: 256000, maxTokens: 131072 },
  'hy3-x':             { efforts: ['low', 'high', 'max'], vision: false, context: 256000, maxTokens: 131072 },
  'hy4-preview':       { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 64000 },
  'hy4-preview-x':     { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 64000 },
  'deepseek-v4-pro':   { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 393216 },
  'deepseek-v4-flash': { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 393216 },
  'deepseek-v4.1-flash': { efforts: ['low', 'high', 'max'], vision: true, context: 1000000, maxTokens: 128000 },
  'glm-5.3':           { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 131072 },
  'glm-5.3-flash':     { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 131072 },
  'glm-5.2':           { efforts: ['low', 'high', 'max'], vision: false, context: 1000000, maxTokens: 131072 },
  'glm-5.1':           { efforts: ['low', 'high', 'max'], vision: false, context: 202745, maxTokens: 131072 },
  'glm-5v-turbo':      { efforts: ['low', 'high', 'max'], vision: true, context: 200000, maxTokens: 131072 },
  'glm-4.6v':          { efforts: ['low', 'high', 'max'], vision: false, context: 128000, maxTokens: 32000 },
  'kimi-k3-1':         { efforts: ['low', 'high', 'max'], vision: true, context: 1000000, maxTokens: 32000 },
  'kimi-k2.7':         { efforts: ['low', 'high', 'max'], vision: false, context: 256000, maxTokens: 32000 },
  'kimi-k2.6':         { efforts: ['low', 'high', 'max'], vision: false, context: 256000, maxTokens: 32000 },
  'kimi-k2.5':         { efforts: ['low', 'high', 'max'], vision: false, context: 256000, maxTokens: 32000 },
  ...
  'minimax-m2.7':      { efforts: ['low', 'high', 'max'], vision: false, context: 204800, maxTokens: 48000 },
}
...
function toModelEntry(m) {
  ...
  return {
    ...
    // 图像请求预算：上游 /v3/config 或 models.json 有声明时透传（供 resolveRequestImagePolicy
    // 使用）；未声明则留空，走官方默认值（640000 px / 1 MiB）
    ...(m.imagePixelBudget !== undefined || stat?.imagePixelBudget !== undefined
      ? { imagePixelBudget: stat?.imagePixelBudget ?? m.imagePixelBudget } : {}),
    ...(m.imageMaxBytes !== undefined || stat?.imageMaxBytes !== undefined
      ? { imageMaxBytes: stat?.imageMaxBytes ?? m.imageMaxBytes } : {}),
  }
}
...
const REASONING_LEVELS = [
  ...
  { id: 'max', name: 'Max', description: '极限思考（默认，最耗额度）' },
]
const DEFAULT_REASONING = 'max'
/** dsh 推理等级 → 上游 reasoning_effort 词汇；off = 不发该字段。
 *  上游实测接受 low/high/max 三档（deepseek-v4.1-flash 三档均 200 且有思考产出，
 *  难题下 max 产出思考显著多于 high），故 max 原样下发，不再降级成 high。 */
const REASONING_WIRE_MAP = { off: undefined, low: 'low', high: 'high', max: 'max' }
 ...
 *  仅在模型自身未声明 maxTokens 时生效——models.json / BUILTIN_CAPABILITIES
 *  的 per-model 值优先，故低上限模型（如 glm-4.6v 实测上限 32000，
 *  超限会被上游以 11102 拒绝）仍走自己的数值，不被这个兜底放大。 */
...
/** 图片输入预算与超限处置：与官方 dsh-llm-deepseek 完全对齐（见「图像预算」一节）。
...
/** base64 表示下的字节长度（与核心 base64Length 同式） */
function base64Length(bytes) { return Math.ceil(bytes / 3) * 4 }

/** 收集请求序（含嵌套 tool-result）的图像字节长度；与核心 collectImageLengths 同语义 */
function collectImageLengths(blocks, lengths, policy) {
  for (const block of blocks || []) {
    if (block.type === 'image') {
      const bytes = policy.byteLength === undefined ? block.attachment.bytes : policy.byteLength(block.attachment)
...
/** 超出预算时需移除的前缀张数；与核心 offloadedImagePrefixCount 同语义（含量化步进） */
...
/** 替换最老的 N 张图为占位文本（不改动原消息）；与核心 replaceOldestImages 同语义 */
function replaceOldestImages(blocks, remaining, placeholder) {
  ...
  for (const [index, block] of (blocks || []).entries()) {
    ...
    if (next !== undefined) next.push(block)
  }
  return next ?? blocks
}

/** 占位符文案兜底（核心不可解析时用）；与核心 offloadedImageText 同文案 */
function offloadedImageTextFallback(ref, access) {
  const identity = `image omitted to fit request image limits; ${imageIdentityFallback(ref)}.`
  if (access === undefined) return `[${identity} No local normalized image path is available; ask the user to attach it again if needed.]`
  return `[${identity} Normalized copy (read-only; may be resized or re-encoded): "${access.readonlyPath}" (${ref.width}x${ref.height}px, ${ref.mediaType}). Source dimensions, format, and byte size may differ.]`
}
function imageIdentityFallback(ref) {
  return ref?.name === undefined ? String(ref?.attachmentId) : `"${ref.name}" (${ref.attachmentId})`
}

/** 本地忠实复刻；仅在核心 `offloadRequestImagesWithPolicy` 不可用时兜底。 */
function offloadImagesLocal(messages, policy) {
  const lengths = []
 ...
 *  故按 dsh 安装位置解析（与 opencode-zen 定位 sharp 同一手法）。加载失败返回 null，
 *  图像处置自动退回本地复刻，行为与官方一致。 */
let _coreLlm
...
function coreLlm() {
  ...
  try {
    ...
    try {
      // 退路：直接按路径 require 核心入口（Node 的 require(esm) 支持）
    ...
    for (const choice of choices) {
      ...
      for (const call of delta.tool_calls || []) {
        ...
        if (!block) {
          ...
          // 上游首片若未给 id（部分 OpenAI 兼容网关只在首个 chunk 带 id），
          // 这里生成一个稳定 uuid 兜底，保证整条 tool-call 的 id 一致。
...
class CodeBuddyAdapter {
  ...
  async *stream(options) {
    ...
    // 超限卸载：与官方 DeepSeek adapter 的内联（base64）路径同策略——按请求序最老的图先换占位符。
    // 优先调用核心导出；解析不到核心（link 安装）时退回本地复刻，两者语义一致。
    let requestMessages = messages
    if (imagesPresent && found && inputModalitiesOf(found).includes('image')) {
      const policy = resolveRequestImagePolicy(found)
      const offloadPolicy = {
        representation: 'base64',
        maxBytes: DEFAULT_MAX_INLINE_REQUEST_IMAGE_BYTES,
        maxImages: DEFAULT_MAX_IMAGES_PER_REQUEST,
        byteQuantum: DEFAULT_INLINE_IMAGE_OFFLOAD_BYTE_QUANTUM,
        countQuantum: DEFAULT_IMAGE_OFFLOAD_COUNT_QUANTUM,
        byteLength: (ref) => Math.min(ref.bytes, policy.maxBytes),
        placeholder: (ref) => (coreLlm()?.offloadedImageText
          ? coreLlm().offloadedImageText(ref, undefined)
          : offloadedImageTextFallback(ref, undefined)),
      }
      const core = coreLlm()
      const offloaded = typeof core?.offloadRequestImagesWithPolicy === 'function'
        ? core.offloadRequestImagesWithPolicy(messages, offloadPolicy)
        : offloadImagesLocal(messages, offloadPolicy)
      if (offloaded !== messages) {
        requestMessages = offloaded
        log(this.ctx, 'info', `[${model}] image request exceeded inline budget; oldest images replaced with placeholders (${typeof core?.offloadRequestImagesWithPolicy === 'function' ? 'core' : 'local'})`)
      }
    }

    const wireMessages = await serializeMessages(requestMessages, system, loadImage)
