...
/** 内置模型能力映射（网关实测 + 各模型族公开资料，2026-08-28）：
 *  efforts = 支持的思考档位（词汇 low/high/max），vision = 识图能力。
 *  实测网关接受度比 /v3/config 的保守声明更宽：hy4-preview 上游仅声明 high，
 *  实际 low/max 均 200 且真实产出思考内容；各家族推理模型 max 档全部通过。
 *  优先级：models.json 显式声明 > 本映射 > 上游声明（本映射缺失时才参考）。 */
const BUILTIN_CAPABILITIES = {
  'hy3':               { efforts: ['low', 'high', 'max'], vision: true },
  'hy3-x':             { efforts: ['low', 'high', 'max'], vision: true },
  'hy4-preview':       { efforts: ['low', 'high', 'max'], vision: true },
  'hy4-preview-x':     { efforts: ['low', 'high', 'max'], vision: true },
  'deepseek-v4-pro':   { efforts: ['low', 'high', 'max'], vision: true },
  'deepseek-v4-flash': { efforts: ['low', 'high', 'max'], vision: true },
  'glm-5.3':           { efforts: ['low', 'high', 'max'], vision: true },
  'glm-5.3-flash':     { efforts: ['low', 'high', 'max'], vision: true },
  'glm-5.2':           { efforts: ['low', 'high', 'max'], vision: true },
  'glm-5.1':           { efforts: ['low', 'high', 'max'], vision: true },
  'glm-5v-turbo':      { efforts: ['low', 'high', 'max'], vision: true },
  'glm-4.6v':          { efforts: ['low', 'high', 'max'], vision: true },
  'kimi-k3-1':         { efforts: ['low', 'high', 'max'], vision: true },
  'kimi-k2.7':         { efforts: ['low', 'high', 'max'], vision: true },
  'kimi-k2.6':         { efforts: ['low', 'high', 'max'], vision: true },
  'kimi-k2.5':         { efforts: ['low', 'high', 'max'], vision: true },
  'kimi-k2-thinking':  { efforts: ['low', 'high', 'max'], vision: false },
  'minimax-m3':        { efforts: ['low', 'high', 'max'], vision: true },
  'minimax-m2.7':      { efforts: ['low', 'high', 'max'], vision: true },
}

/** 内置默认表：仅当 models.json 缺失或损坏时兜底使用
 *  （能力位由内置映射 applyBuiltinCapabilities 补齐） */
const DEFAULT_MODELS = [
  { id: 'hy3', name: 'Hunyuan 3', contextWindow: 190000, description: '腾讯混元 Hy3' },
  { id: 'deepseek-v4-pro', name: 'DeepSeek V4 Pro', contextWindow: 128000, description: 'DeepSeek V4 Pro' },
  { id: 'deepseek-v4-flash', name: 'DeepSeek V4 Flash', contextWindow: 128000, description: 'DeepSeek V4 Flash' },
]
...
/**
 * 静态条目能力位补齐：未显式声明 reasoningEfforts / input 的，按内置映射
 * 自动补上（思考档位、识图都不用手工配）；显式声明（含 null = 关闭）不覆盖。
 */
...
function toModelEntry(m) {
  ...
  return {
    id: m.id,
    name: m.name || m.id,
    contextWindow: Number(m.maxInputTokens),
    maxTokens: Number(m.maxOutputTokens) || undefined,
